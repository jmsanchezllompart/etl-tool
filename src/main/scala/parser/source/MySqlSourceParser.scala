...
import core.source.{DataSource, MySqlSource}
...
/**
 * Parser implementation for constructing a [[MySqlSource]] from a JSON configuration.
 *
 * This parser reads required fields from a Circe [[io.circe.ACursor]] and converts them
 * into a fully configured [[MySqlSource]] instance.
 *
 ...
 */
object MySqlSourceParser extends Parser[DataSource] {
  /** Identifier for this parser, used in parser registries. */
  override def name: String = "MySqlSource"

  /**
   * Parses a [[MySqlSource]] from the given JSON cursor.
   *
   * @param cursor The Circe [[io.circe.ACursor]] pointing to the JSON object
   *               representing a MySQL data source configuration.
   * @return A fully constructed [[MySqlSource]] instance.
   *
  ...
  override def parse(cursor: ACursor): DataSource = {
    val host = cursor.get[String]("Host") match {
      ...
      case Left(error) => throw new IllegalArgumentException(
        s"[MySqlSourceParser] Missing or invalid 'Host' field: ${error.getMessage}"
      )
    }
    ...
    val port = cursor.get[Int]("Port") match {
      ...
      case Left(error) => throw new IllegalArgumentException(
        s"[MySqlSourceParser] Missing or invalid 'Port' field (expected Int): ${error.getMessage}"
      )
    }
    ...
    val database = cursor.get[String]("Database") match {
      ...
      case Left(error) => throw new IllegalArgumentException(
        s"[MySqlSourceParser] Missing or invalid 'Database' field: ${error.getMessage}"
      )
    }
      ...
      try {
        ...
        case e: Exception =>
          throw new IllegalArgumentException(
            s"[MySqlSourceParser] Failed to parse 'Auth' field: ${e.getMessage}",
            e
          )
      }
    ...
    val query = cursor.get[String]("RawQuery") match {
      ...
      case Left(error) => throw new IllegalArgumentException(
        s"[MySqlParser] Missing or invalid 'RawQuery' field: ${error.getMessage}"
      )
    }
